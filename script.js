(() => {
  ...
  function closeMenus() {
    ...
    document.querySelectorAll(".dropdown-toggle").forEach(button => button.setAttribute("aria-expanded", "false"));
  }
  ...
  document.querySelectorAll(".dropdown-toggle").forEach(button => {
    button.addEventListener("click", event => {
      if (window.matchMedia("(max-width: 900px)").matches) {
        ...
        dropdown?.parentElement?.querySelectorAll(":scope > .dropdown.open").forEach(item => {
          if (item !== dropdown) {
  ...
  document.querySelectorAll("[data-category-link]").forEach(link => {
    link.addEventListener("click", event => {
      ...
      if (link.tagName === "BUTTON" && window.matchMedia("(max-width: 900px)").matches && dropdown?.querySelector(":scope > .dropdown-content")) return;
      event.preventDefault();
